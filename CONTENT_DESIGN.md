...
|---|---|---|---|
| 1 | Cover / Landing screen | **On** | Couple's names, "tap to open," personalized greeting placeholder (§4). No wedding date on the Cover (removed 2026-08-15) — the date shows on Countdown only. |
...
| 8 | Digital gift info | **Off** | Bank/e-wallet info, copy-to-clipboard, optional QR — off by default (revised). |
...
| `opening/` | Cover, revealed in-flow hero | `background_opening.png` + `arch_opening.png` (floral arch framing the greeting). **Why two folders for one section:** the current Cover component renders near-identical content twice — once in the pre-tap fixed overlay, once in-flow underneath (CONTENT_DESIGN.md §4, Cover.tsx). Giving each state its own asset folder makes "before you tap" and "after you tap" visually distinct instead of a flat swap. Background also reused for **Wishes** (no dedicated folder). |
...
| `components/` | Love Story, Gallery, Wishes, Gift (accents); Cover + Countdown (`arch_flower1_left/right.png`, §30) | Standalone flowers (`lily.png`, `orchid.png`, `amaranthus.png`, `mainbouquet.png`), `birds.png`, `lace banner.png` (a reusable translucent panel — same shape as the cover banner but without birds, good as a heading backdrop), two silhouette assets, and (Phase 8) `arch_flower1_left.png`/`_right.png` — the split arch halves, moved here from `opening/` since they're now shared between Cover and Countdown rather than Cover-specific. Used as corner/heading decoration only — no functional changes to these sections. |
...
The pre-tap "Tap to Open" / "Buka" button now only renders once all of the pre-tap overlay's images (`background_cover`, `landscape_cover`, `birds_banner_cover`, the `name/` combined signature) have fired their `onLoad` callback — previously the button appeared immediately on mount regardless of whether the artwork behind it had actually finished loading, which could show a mostly-blank overlay with a working button on a slow connection. A small loading indicator (spinner + "Memuat..."/"Loading..." label, `labels.cover.loading`) shows in the button's place until then.
