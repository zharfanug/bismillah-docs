...
- Personalized greeting placeholder (CONTENT_DESIGN.md §4). **Fixed (2026-08-17, CONTENT_DESIGN.md §56):** a long `?to=` name used to be able to spill past the visible lace fabric onto the birds/background around it — the greeting text's sizing now tracks `birds_lace_banner.png`'s actual rendered box (via CSS container-query units) instead of assuming a fixed relationship to its container, and is capped to the lace's real ~51% width. Visually confirmed by the user.
